...
- 提供实验性的只读 Rust API，用于有边界地递归解码旧版属性值，并限制深度、节点数和集合数量，同时显式使用 Palworld 映射类型提示。
- 提供初步且顺序确定的实体索引，覆盖角色实例、公会、据点、物品容器和角色容器，并包含循环安全的依赖遍历和稳定的校验问题代码。
- 有边界地解码公会关系 `RawData`，校验内嵌公会 ID，并提取玩家、角色实例、据点和据点地图对象引用。
...
- `PlM`/Oodle 解压、UE 5.4 完整属性标签，以及其余 Palworld 自定义二进制编解码器。
- 完整实体覆盖、跨文件玩家索引和依赖完整的校验。实验性分析 API 会报告不支持的自定义数据和未解析关系，不会声称依赖图完整。
...
命令只读取文件、计算哈希、验证受支持的 zlib 容器，并报告有边界的 GVAS 头及顶层属性标签元数据，不会修改存档。当前 CLI 仍会跳过属性值；递归解码和依赖分析目前是实验性的 Rust 库 API，尚未加入 CLI 输出。JSON 使用 `schema_version: 1`、稳定错误码和不翻译的机器字段。
...
- `palmerge-parser`：有边界的世界存档发现、保守格式探测、受资源限制的 zlib 校验、GVAS 元数据与旧版属性解码，以及初步的 Palworld 实体索引、依赖图和校验基础类型。
