...
命令只读取文件、计算哈希、验证受支持的 zlib 容器并报告有边界的 GVAS 头元数据，不会修改存档。JSON 使用 `schema_version: 1`、稳定错误码和不翻译的机器字段。
...
- `palmerge-parser`：有边界的世界存档发现、保守格式探测、受资源限制的 zlib 校验和 GVAS 元数据解析。
