...
pub fn fingerprint(path: &Path) -> Result<Fingerprint, PalError> {
    ...
    let sha256 = sha256_reader(&mut file).map_err(|error| {
        PalError::new(ErrorCode::Io, format!("{}: {error}", path.display()))
    })?;
...
impl Sha256 {
    ...
    fn update(&mut self, mut input: &[u8]) {
        self.bit_len = self.bit_len.wrapping_add((input.len() as u64).wrapping_mul(8));
    ...
    fn compress(&mut self, block: &[u8; 64]) {
        ...
        for index in 0..64 {
            ...
            let choice = (e & f) ^ ((!e) & g);
            let temp1 = h.wrapping_add(s1).wrapping_add(choice).wrapping_add(K[index]).wrapping_add(w[index]);
...
mod tests {
    ...
    fn sha256_matches_standard_vectors() {
        assert_eq!(sha256_reader(&mut &b""[..]).unwrap(), "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
        assert_eq!(sha256_reader(&mut &b"abc"[..]).unwrap(), "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
    }

    #[test]
    fn important_messages_are_localized() {
        assert_eq!(message(Locale::English, MessageKey::UnknownFormat), "unknown or unsupported save format");
        assert_eq!(message(Locale::SimplifiedChinese, MessageKey::UnknownFormat), "未知或不支持的存档格式");
    }
}
